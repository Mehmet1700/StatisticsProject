...
# Calculate cluster-robust standard errors using vcovHC
# For panel models, cluster = "group" clusters by the panel unit (country)
robust_se_pooled <- sqrt(diag(vcovHC(pooled_ols, type = "HC1", cluster = "group")))
robust_se_fe <- sqrt(diag(vcovHC(fe_model, type = "HC1", cluster = "group")))
robust_se_re <- sqrt(diag(vcovHC(re_model, type = "HC1", cluster = "group")))
...
stargazer(
  ...
  se = list(robust_se_pooled, robust_se_fe, robust_se_re),
  notes = "Cluster-robust standard errors (HC1) in parentheses",
  notes.append = FALSE
)
```
...
This section automatically generates the PDF poster based on the analysis results.

```{r generate-poster, include=FALSE}
# ==============================================================================
# AUTOMATED POSTER GENERATION (FIXED INDENTATION)
# ==============================================================================
...
# 1. PREPARE DATA & PLOTS FOR POSTER
# ----------------------------------

# A) Calculate Robust SEs
cov_fe <- vcovHC(fe_model, type = "HC1", cluster = "group")
robust_se_fe <- sqrt(diag(cov_fe))

# B) Create the "Forest Plot"
model_data <- broom::tidy(fe_model) %>%
  mutate(
    std.error = robust_se_fe,
...
coef_plot <- ggplot(model_data, aes(x = estimate, y = term)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2, color = "#2c3e50", linewidth=1) +
  geom_point(size = 4, color = "#B5CE45") + 
  labs(x = "Effect on GDP Growth (%)", y = "") +
  theme_minimal(base_size = 12) +
  theme(
...
# C) Create Summary Tables for Poster - Using actual values from analysis

# Table 1: Diagnostic Tests Summary (from Section 10)
# These are the actual test results that guide model selection
diagnostic_table <- data.frame(
  ...
  Test = c("Breusch-Pagan (Heteroskedasticity)", 
           ...
           "Breusch-Pagan LM (RE vs. OLS)", 
           "F-Test (FE vs. OLS)"),
  Statistic = c("1.70", "28.27", "28.27", "1.52", "15.86"),
  P_Value = c("0.637", "< 0.001", "< 0.001", "0.218", "< 0.001"),
  Decision = c("Homoskedastic", "Use FE", "Use FE", "OLS OK", "FE needed"),
  stringsAsFactors = FALSE
)

# Table 2: Model Performance Comparison (from Section 11)
# Comparing R-squared, observations, and fit statistics across models
model_performance <- data.frame(
  Model = c("Pooled OLS", "Fixed Effects", "Random Effects"),
  R_Squared = c("0.027", "0.016", "0.020"),
  Adj_R_Squared = c("0.016", "0.066", "0.019"),
  Observations = c(269, 269, 269),
  stringsAsFactors = FALSE
)

# D) Create evolution plot for 4 selected countries
selected_countries <- c("PT", "DE", "FR", "ES")
selected_names <- c(
...
expl_vars_data <- evolution_data %>%
  ...
  mutate(
    Variable = case_when(
      Variable == "migration_rate" ~ "Migration",
      Variable == "unemployment_rate" ~ "Unemployment",
      Variable == "investment_rate" ~ "Investment"
    ),
    Variable = factor(Variable, levels = c("Migration", "Unemployment", "Investment"))
  )
...
evolution_plot <- ggplot() +
  ...
  geom_line(data = gdp_data,
            aes(x = time, y = gdp_pc_growth),
            color = "#7B3294", linewidth = 0.8, alpha = 0.8) +
  geom_point(data = gdp_data,
             aes(x = time, y = gdp_pc_growth),
             color = "#7B3294", size = 1.5, shape = 17, alpha = 0.8) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40", linewidth = 0.3) +
  facet_wrap(~ country_label, ncol = 2, scales = "free_y") +
  scale_x_continuous(breaks = seq(2014, 2023, 3)) +
  scale_y_continuous(name = "") +
  scale_color_manual(values = c("Migration" = "#2166AC", "Unemployment" = "#B2182B", "Investment" = "#1B7837")) +
  scale_linetype_manual(values = c("Migration" = "solid", "Unemployment" = "dashed", "Investment" = "dotted")) +
...
# E) Save data
save(fe_model, panel_data, robust_se_fe, coef_plot, diagnostic_table, model_performance, evolution_plot, file = "data_group_09.RData")

# 2. DEFINE CSS CONTENT (EXTERNAL FILE)
# ------------------------------------------------------------
css_content <- '/* Professional Academic Poster CSS - Group 09 */
/* ============================================== */
...
@media print {
  ...
  .poster { width: 297mm; height: 210mm; }
}
...
/* Override RMarkdown default container styles */
...
/* Poster Container - Full Viewport */
.poster { 
  width: 100vw;
  height: 100vh;
...
/* Multi-Column Flow Container (Newspaper Style) */
.poster-body {
  column-count: 3;
  column-gap: 1.5cm;
  column-fill: auto;
  flex: 1;
  margin-top: 8px;
  height: calc(100vh - 120px); /* Fixed height for column breaks to work */
  overflow: hidden;
}

/* Section Styles - Allow natural column flow */
.section {
  background: white;
  border-radius: 5px;
  padding: 8px 10px;
...
/* Results Grid Layout - 2 columns inside Results section */
.results-grid {
  display: grid;
...
writeLines(css_content, "poster_group_09.css")

# 3. DEFINE POSTER CONTENT (NO INDENTATION TO PREVENT CODE BLOCKS)
# ------------------------------------------------------------
bt3 <- paste0("`", "`", "`") 

poster_content <- paste(
"---",
"output: html_document",
"---",
"",
paste0(bt3, "{r, include=FALSE}"),
...
paste0(bt3, "{r fig.width=4, fig.height=2.2, fig.align=\"center\"}"),
...
# 4. WRITE AND RENDER
# -------------------
# We save the poster script as a separate file
writeLines(poster_content, "poster_group_09.Rmd")

# 1. Create HTML first 
rmarkdown::render("poster_group_09.Rmd", output_file = "poster_group_09.html", quiet = TRUE)

# 2. Try to generate PDF automatically
tryCatch({
  pagedown::chrome_print("poster_group_09.html", output = "poster_group_09.pdf")
  message("SUCCESS: poster_group_09.pdf has been created!")
}, error = function(e) {
  # If it fails (e.g. Chrome not found), just warn
  warning("PDF could not be generated automatically")
  message("INSTRUCTION: Open the file 'poster_group_09.html' in your browser and select 'Print -> Save as PDF'.")
})